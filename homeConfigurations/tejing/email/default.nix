...
in
{
  ...
  xsession.windowManager.i3.config.startup = [{ command = "${my.launch.term} app neomutt ${pkgs.writeShellScript "neomutt-cycle" "while true; do neomutt;done"}"; always = false; notification = false; }];
}
